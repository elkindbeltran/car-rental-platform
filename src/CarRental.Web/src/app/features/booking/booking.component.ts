  ...
  template: `
    ...
    <section class="page booking-layout">
      <form class="booking-form" [formGroup]="form" (ngSubmit)="submit()">
        ...
        <div class="step"><span>03</span><div><h2>Who's driving?</h2><p>Choose the customer attached to this reservation.</p></div></div>
        <label>Customer<select formControlName="customerId"><option value="">Select a customer</option>@for(c of customers();track c.id){<option [value]="c.id">{{c.firstName}} {{c.lastName}} — {{c.email}}</option>}</select><small>Customer selection is available to administrators.</small></label>
...
export class BookingComponent {
  private readonly api=inject(ApiService);private readonly fb=inject(FormBuilder);private readonly route=inject(ActivatedRoute);
  readonly vehicles=signal<VehicleSummary[]>([]);readonly customers=signal<CustomerSummary[]>([]);readonly selectedVehicle=signal<VehicleSummary|null>(null);readonly error=signal('');readonly submitting=signal(false);readonly booking=signal<Booking|null>(null);
  ...
  constructor(){forkJoin({vehicles:this.api.vehicles(1,100,'',VehicleStatus.Available),customers:this.api.customers(1,100)}).subscribe({next:r=>{this.vehicles.set(r.vehicles.items);this.customers.set(r.customers.items);this.selectVehicle()},error:e=>this.error.set(e.status===403?'An Administrator role is required to choose a customer and create a booking in the current API.':apiErrorMessage(e))});this.form.valueChanges.subscribe(()=>this.error.set(''))}
